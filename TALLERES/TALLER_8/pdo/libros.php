...
if(isset($_POST['guardar'])){
  $stmt = $pdo->prepare("INSERT INTO libros (titulo, autor, isbn, anio, cantidad) VALUES (?, ?, ?, ?, ?)");
  $stmt->execute([$_POST['titulo'], $_POST['autor'], $_POST['isbn'], $_POST['anio'], $_POST['cantidad']]);
}
...
$res = $pdo->query("SELECT * FROM libros");
?>
<h2>Libros</h2>
<form method="post">
...
<?php while($l = $res->fetch(PDO::FETCH_ASSOC)): ?>
<tr>
<td><?=$l['id']?></td><td><?=$l['titulo']?></td><td><?=$l['autor']?></td>
<td><?=$l['isbn']?></td><td><?=$l['anio']?></td><td><?=$l['cantidad']?></td>
